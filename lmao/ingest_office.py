...
class OfficeIngester:
    def __init__(self, client_id: str, client_secret: str, tenant_id: str, site_id: str, user_email: str, download_dir: str, source: str, batch_size: int = 10) -> None:
        ...
        self.access_token = self.get_access_token() if source == 'sharepoint' else None
    ...
    def list_sharepoint_documents(self) -> Dict[str, Any]:
        ...
        drive_items_url = f"https://graph.microsoft.com/v1.0/sites/{self.site_id}/drive/root/children"
    ...
    def filter_documents_by_user(self, drive_items: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        for item in drive_items["value"]:
            ...
            for version in versions["value"]:
                if "lastModifiedBy" in version and version["lastModifiedBy"]["user"]["email"] == self.user_email:
    ...
    def list_local_documents(self) -> List[str]:
        """
        List documents in the local directory.

        :return: A list of file paths.
        """
        supported_extensions = ('.docx', '.pptx', '.xlsx')
        documents = [os.path.join(self.download_dir, f) for f in os.listdir(self.download_dir) if f.endswith(supported_extensions)]
        return documents
    ...
    def run(self) -> None:
        """
        Run the OfficeIngester to download and process documents from SharePoint or local directory.
        """
        if self.source == 'sharepoint':
            drive_items = self.list_sharepoint_documents()
            documents = self.filter_documents_by_user(drive_items)
        else:
            documents = self.list_local_documents()
